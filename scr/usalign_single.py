...
# arguments
parser = argparse.ArgumentParser(description='Compute TM-scores between experimental and model PDB files using USalign (-d 5 for single RNAs).')
parser.add_argument('--afm_dir', required=True, help='Directory containing predicted PDB files')
parser.add_argument('--exp_dir', required=True, help='Directory containing ground truth PDB files')
...
# iterate and compute usalign, extract the score
for exp_pdb_file in os.listdir(exps_dir):
    if exp_pdb_file.endswith('.pdb'):
        ...
        if os.path.exists(model_pdb_path):
            command = f"USalign {model_pdb_path} {exp_pdb_path} -mol RNA -d 5"
